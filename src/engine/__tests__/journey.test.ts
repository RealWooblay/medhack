...
describe('PharmCAT report JSON handoff', () => {
  it('preserves the PharmCAT version without treating an outside call as a validated caller manifest', async () => {
    ...
    const contents = JSON.stringify({
      ...
      drugs: {
        'CPIC Guideline Annotation': {
          citalopram: { name: 'citalopram', guidelines: [] },
        },
      },
    })
    ...
    expect(report.pharmcatVersion).toContain('v3.3.0-8-g8ff5870f')
    const cyp2d6 = report.genes.find((gene) => gene.gene === 'CYP2D6')!
    expect(cyp2d6.diplotype).toBe('*1/*4')
    expect(cyp2d6.structuralVariationUnresolved).toBe(true)
    ...
    expect(cyp2d6.missingPositionLabels.join(' ')).toMatch(/does not identify or validate/i)
  })
})
